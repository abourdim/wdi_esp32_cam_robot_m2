/*
  ...
  Motor experimentation controls:
    ...
    - 16-step PWM soft start with staggered two-motor startup.
  ...
  Network fallback:
    ...
    - Fallback runs AP-only for a stable 192.168.4.1 connection.
...
// Firmware release is embedded directly in this sketch so the .ino is
// self-contained. Do not move this into a separate header: users may copy
// just the sketch into an existing PlatformIO project. This line is the only
// place the version lives, so a release is one edit here.
#define ROBOT_FIRMWARE_VERSION "v2.0"
...
static bool wifiApplyInProgress = false;
static unsigned long wifiApplyStartedMs = 0;
static const uint32_t WIFI_APPLY_TIMEOUT_MS = 12000;
...
// Automatic background retries are deliberately disabled while the fallback
// AP is active. AP and STA share one ESP32 radio; background scans/association
// attempts can make 192.168.4.1 unreachable just as a browser connects.
...
static const char *FIRMWARE_VERSION = ROBOT_FIRMWARE_VERSION;
...
// LEDC ownership is explicit for every PWM output, and it has to be, because
// the Arduino core hard-wires channel to group and timer -- there is no
// allocator to negotiate with:
//
//   group = chan / 8        (0 = high speed, 1 = low speed)
//   timer = (chan / 2) % 4
//
// esp32-camera drives XCLK from LEDC_TIMER_0 in LEDC_LOW_SPEED_MODE, and
// reprograms that timer to 20 MHz at 1-bit duty resolution. Channels 8 and 9
// are low-speed timer 0. The motors used to sit on 8..11, so esp_camera_init()
// silently dropped the LEFT motor to a 1-bit timer: ledcWrite() kept accepting
// duty values, every non-zero one saturated the comparator and pinned the pin
// HIGH, the 16-step soft start did nothing, and the resulting full-current
// slam browned the board out on the first left-motor command -- while the
// right motor on timer 1 soft-started perfectly, every time.
//
// Current allocation, all disjoint:
//   camera XCLK   low-speed   channel 0       timer 0
//   flash LED     high-speed  channel 7       timer 3
//   motor RIGHT   low-speed   channels 10-11  timer 1
//   motor LEFT    low-speed   channels 12-13  timer 2
//
// The static_asserts below keep it that way; verifyMotorLedcTimers() checks
// the same thing at runtime, after every camera init, in case a future core
// or camera version moves a timer without moving these numbers.
#define LEDC_CHAN_GROUP(chan) ((chan) / 8)
#define LEDC_CHAN_TIMER(chan) (((chan) / 2) % 4)

// esp32-camera's XCLK timer. Nothing else may share it.
#define CAMERA_XCLK_LEDC_GROUP 1  /* LEDC_LOW_SPEED_MODE */
#define CAMERA_XCLK_LEDC_TIMER 0  /* LEDC_TIMER_0 */
#define LEDC_CHAN_CLEAR_OF_CAMERA(chan) (!(LEDC_CHAN_GROUP(chan) == CAMERA_XCLK_LEDC_GROUP && LEDC_CHAN_TIMER(chan) == CAMERA_XCLK_LEDC_TIMER))
...
static const uint8_t MOTOR_L_IN1_LEDC_CHANNEL = 12;
static const uint8_t MOTOR_L_IN2_LEDC_CHANNEL = 13;
static const uint8_t MOTOR_R_IN1_LEDC_CHANNEL = 10;
static const uint8_t MOTOR_R_IN2_LEDC_CHANNEL = 11;
static const uint8_t MOTOR_LEDC_BITS = 8;
static const uint32_t MOTOR_LEDC_FREQ_HZ = 1000;

static_assert(LEDC_CHAN_CLEAR_OF_CAMERA(MOTOR_L_IN1_LEDC_CHANNEL),
              "motor L IN1 shares the camera XCLK LEDC timer");
static_assert(LEDC_CHAN_CLEAR_OF_CAMERA(MOTOR_L_IN2_LEDC_CHANNEL),
              "motor L IN2 shares the camera XCLK LEDC timer");
static_assert(LEDC_CHAN_CLEAR_OF_CAMERA(MOTOR_R_IN1_LEDC_CHANNEL),
              "motor R IN1 shares the camera XCLK LEDC timer");
static_assert(LEDC_CHAN_CLEAR_OF_CAMERA(MOTOR_R_IN2_LEDC_CHANNEL),
              "motor R IN2 shares the camera XCLK LEDC timer");
static_assert(LEDC_CHAN_CLEAR_OF_CAMERA(LED_LEDC_CHANNEL),
              "flash LED shares the camera XCLK LEDC timer");

// Both inputs of one motor must stay on one timer, so IN1 and IN2 cannot end
// up at different PWM frequencies across a direction change.
static_assert(LEDC_CHAN_TIMER(MOTOR_L_IN1_LEDC_CHANNEL) == LEDC_CHAN_TIMER(MOTOR_L_IN2_LEDC_CHANNEL),
              "motor L inputs are on different LEDC timers");
static_assert(LEDC_CHAN_TIMER(MOTOR_R_IN1_LEDC_CHANNEL) == LEDC_CHAN_TIMER(MOTOR_R_IN2_LEDC_CHANNEL),
              "motor R inputs are on different LEDC timers");
static_assert(LEDC_CHAN_GROUP(MOTOR_L_IN1_LEDC_CHANNEL) == LEDC_CHAN_GROUP(MOTOR_L_IN2_LEDC_CHANNEL) &&
              LEDC_CHAN_GROUP(MOTOR_R_IN1_LEDC_CHANNEL) == LEDC_CHAN_GROUP(MOTOR_R_IN2_LEDC_CHANNEL),
              "motor inputs are split across LEDC speed groups");
...
// Keep the camera configuration after setup so a transient sensor/DMA lockup
// can be recovered without rebooting the whole robot. Motor brush noise and
// marginal camera ribbon contacts can make esp_camera_fb_get() return NULL;
// the old code immediately killed the MJPEG socket, which made the browser
// reconnect forever. The stream now retries and, when necessary, power-cycles
// and reinitializes the camera in place.
static camera_config_t cameraConfig = {};
static SemaphoreHandle_t cameraMutex = nullptr;

// Small RAII guard for camera control/status handlers. The MJPEG stream,
// settings API and recovery task run on different FreeRTOS workers; sensor
// register access must not race a framebuffer capture or driver reinit.
class ScopedCameraLock {
 public:
  explicit ScopedCameraLock(TickType_t waitTicks) : locked_(false) {
    if (cameraMutex != nullptr) {
      locked_ = (xSemaphoreTake(cameraMutex, waitTicks) == pdTRUE);
    }
  }

  ~ScopedCameraLock() { release(); }
...
static volatile bool cameraRecovering = false;
static volatile uint32_t cameraRecoveryCount = 0;
static volatile uint32_t cameraCaptureFailureCount = 0;

// Camera failures are handled in two stages. First tolerate a couple of
// seconds of missing frames without touching the sensor. If the camera really
// stalls, perform a deliberate power-cycle with long settling times. If that
// still fails, leave the control UI alive and retry in the background instead
// of making the browser hammer port 81 with reconnects.
static const int CAMERA_CAPTURE_RETRIES_BEFORE_RECOVERY = 40;
static const int CAMERA_CAPTURE_RETRY_MS = 50;
static const int CAMERA_RECOVERY_POWERDOWN_MS = 250;
static const int CAMERA_RECOVERY_BOOT_MS = 800;
static const int CAMERA_RECOVERY_INIT_ATTEMPTS = 3;
static const int CAMERA_RECOVERY_ATTEMPT_BACKOFF_MS = 500;
static const int CAMERA_RECOVERY_WARMUP_FRAMES = 2;
static const uint32_t CAMERA_BACKGROUND_RECOVERY_INTERVAL_MS = 5000;
static unsigned long lastCameraRecoveryAttemptMs = 0;
...
// This cannot repair a physically faulty motor, but it minimizes the current
// step and EMI presented to the ESP32-CAM.
static const int MOTOR_SOFT_START_STEPS = 16;
static const int MOTOR_SOFT_START_STEP_MS = 12;
static const int MOTOR_START_STAGGER_STEPS = 5;  // second motor starts ~60 ms later
...
// the gearbox and creates another avoidable current spike.
...
// HTTP handlers normally run on core 0 while Arduino loop() runs on core 1.
// Both paths can stop/apply motors (commands, timeout, OTA), so serialize every
// LEDC write and the associated current* state. Holding this mutex across the
// short soft-start/reversal delays prevents two cores from interleaving bridge
// states and producing an invalid transient.
...
static bool startFallbackAccessPoint() {
  ...
  // IMPORTANT: fallback means AP-only. The ESP32 has one 2.4 GHz radio; when
  // AP+STA mode scans or tries to associate, the radio leaves the AP channel.
  // On Windows/Chrome this showed up as ERR_CONNECTION_TIMED_OUT at
  // 192.168.4.1 even though the HTTP server had already started. Turn the
  // failed station completely off and keep the radio pinned to the robot AP.
  ...
  // Fixed channel 1, visible SSID, up to four clients. The default gateway is
  // 192.168.4.1; setting the channel explicitly prevents a stale STA channel
  // from being inherited after the failed infrastructure connection.
  bool ok = WiFi.softAP(fallbackApSsid, FALLBACK_AP_PASSWORD, 1, 0, 4);
  fallbackApActive = ok;
  ...
  snprintf(
    dbg,
    sizeof(dbg),
    "BOOT: Fallback AP IP %u.%u.%u.%u; AP-only channel 1",
  ...
  previousStaStatus = WL_DISCONNECTED;

  setDebugMessage(
    "NET: Automatic configured-Wi-Fi retries disabled while fallback AP is active"
  );
...
static void serviceWiFiFallback() {
  ...
  // The ONLY station association allowed in fallback mode is one explicitly
  // requested from the Settings page. Give it a bounded window, then return
  // to AP-only mode so a wrong SSID/password can never leave 192.168.4.1
  // intermittently unreachable.
  if (wifiApplyInProgress) {
    if (currentStatus == WL_CONNECTED) {
      wifiApplyInProgress = false;
    ...
    if (millis() - wifiApplyStartedMs >= WIFI_APPLY_TIMEOUT_MS) {
      wifiApplyInProgress = false;
      setDebugMessage(
        "NET: Requested Wi-Fi connection timed out; returning to stable fallback AP"
      );
  ...
  // If an explicitly established station link is later lost, immediately
  // return to AP-only rather than letting the Wi-Fi stack hunt in the
  // background. The AP SSID/password stay the same.
  if (previousStaStatus == WL_CONNECTED && currentStatus != WL_CONNECTED) {
    setDebugMessage(
      "NET: Configured Wi-Fi lost; returning to stable fallback AP"
    );
    startFallbackAccessPoint();
    return;
  }

  if (currentStatus == WL_CONNECTED) previousStaStatus = WL_CONNECTED;

  // Deliberately no automatic WiFi.reconnect() here.
}
...
// Updated when the stream starts and after each frame is completely handed to
// the HTTP socket. The browser uses the age of this heartbeat rather than a
// single FPS sample to decide whether a stream is genuinely stuck. This avoids
// reconnect loops on a busy/slow Wi-Fi interval.
static volatile uint32_t streamHeartbeatMs = 0;
...
static void writeTb6612(
  ...
  // Shared PWMA/PWMB is held high while moving, so direction is selected by
  // which direction input carries PWM. Zero the idle side first so there is
  // never a transient where both direction inputs are active.
  const uint8_t driveChannel = reverse ? in2Channel : in1Channel;
  const uint8_t idleChannel = reverse ? in1Channel : in2Channel;

  ledcWrite(idleChannel, 0);
  ledcWrite(driveChannel, pwm);
}

static void stopMotorsUnlocked() {
  // PWMA and PWMB are tied together on this PCB. Keep that shared enable HIGH
  // continuously and stop by driving all four direction inputs to zero. This
  // avoids a large shared-enable edge on every button press and matches the
  // carrier schematic's intended operating mode.
  ledcWrite(MOTOR_L_IN1_LEDC_CHANNEL, 0);
  ledcWrite(MOTOR_L_IN2_LEDC_CHANNEL, 0);
  ledcWrite(MOTOR_R_IN1_LEDC_CHANNEL, 0);
  ledcWrite(MOTOR_R_IN2_LEDC_CHANNEL, 0);
...
static void writeMotorsUnlocked(int leftPWM, int rightPWM, bool reverse) {
  ...
  const bool softStartLeft = (currentLeftPWM == 0 && leftPWM > 0);
  const bool softStartRight = (currentRightPWM == 0 && rightPWM > 0);

  // Clear all direction duties before a zero-to-moving ramp. The shared
  // PWMA/PWMB line is already held HIGH, so starting motion changes only the
  // requested direction input instead of switching the whole H-bridge on.
  if (softStartLeft || softStartRight) {
    ledcWrite(MOTOR_L_IN1_LEDC_CHANNEL, 0);
    ledcWrite(MOTOR_L_IN2_LEDC_CHANNEL, 0);
    ledcWrite(MOTOR_R_IN1_LEDC_CHANNEL, 0);
    ledcWrite(MOTOR_R_IN2_LEDC_CHANNEL, 0);
  ...
  if (softStartLeft || softStartRight) {
    // When both motors start together, delay the right motor by several ramp
    // steps so their initial stall-current pulses are not coincident.
    const bool staggerBoth = softStartLeft && softStartRight;
    const int totalSteps = MOTOR_SOFT_START_STEPS +
                           (staggerBoth ? MOTOR_START_STAGGER_STEPS : 0);

    for (int step = 1; step <= totalSteps; step++) {
      int leftStep = leftPWM;
      int rightStep = rightPWM;

      if (softStartLeft) {
        const int leftIndex = min(step, MOTOR_SOFT_START_STEPS);
        leftStep = (leftPWM * leftIndex + MOTOR_SOFT_START_STEPS - 1) /
                   MOTOR_SOFT_START_STEPS;
      }

      if (softStartRight) {
        int rightIndex = step;
        if (staggerBoth) rightIndex -= MOTOR_START_STAGGER_STEPS;

        if (rightIndex <= 0) {
          rightStep = 0;
        } else {
          rightIndex = min(rightIndex, MOTOR_SOFT_START_STEPS);
          rightStep = (rightPWM * rightIndex + MOTOR_SOFT_START_STEPS - 1) /
                      MOTOR_SOFT_START_STEPS;
        }
      }

      writeMotorPairUnlocked(leftStep, rightStep, reverse);

      if (step < totalSteps) {
        delay(MOTOR_SOFT_START_STEP_MS);
      }
    }
  } else {
    writeMotorPairUnlocked(leftPWM, rightPWM, reverse);
  }
...
// Configure the four motor direction channels and bind them to their pins.
// Split out of setup() because esp_camera_init() reprograms LEDC timers -- at
// boot and again on every in-place sensor recovery -- and this is what puts
// the motors back if it ever reaches one of theirs.
static bool applyMotorLedcSetup() {
  const bool ok =
    ledcSetup(MOTOR_L_IN1_LEDC_CHANNEL, MOTOR_LEDC_FREQ_HZ, MOTOR_LEDC_BITS) != 0 &&
    ledcSetup(MOTOR_L_IN2_LEDC_CHANNEL, MOTOR_LEDC_FREQ_HZ, MOTOR_LEDC_BITS) != 0 &&
    ledcSetup(MOTOR_R_IN1_LEDC_CHANNEL, MOTOR_LEDC_FREQ_HZ, MOTOR_LEDC_BITS) != 0 &&
    ledcSetup(MOTOR_R_IN2_LEDC_CHANNEL, MOTOR_LEDC_FREQ_HZ, MOTOR_LEDC_BITS) != 0;

  if (ok) {
    ledcAttachPin(MOTOR_L_IN1_PIN, MOTOR_L_IN1_LEDC_CHANNEL);
    ledcAttachPin(MOTOR_L_IN2_PIN, MOTOR_L_IN2_LEDC_CHANNEL);
    ledcAttachPin(MOTOR_R_IN1_PIN, MOTOR_R_IN1_LEDC_CHANNEL);
    ledcAttachPin(MOTOR_R_IN2_PIN, MOTOR_R_IN2_LEDC_CHANNEL);
  }

  return ok;
}

// Read back what the motor timers are actually running at, and say so.
//
// A motor channel whose timer has been reprogrammed behind its back does not
// fail loudly: ledcWrite() keeps returning normally and the duty quietly
// saturates. That is exactly how a 1-bit 20 MHz XCLK timer turned every
// left-motor command into a full-duty slam and a brownout reset, with nothing
// in the log between "Command: RIGHT" and the boot banner. The static_asserts
// on the channel numbers prevent the known collision; this catches a core or
// camera-driver update that moves a timer out from under them anyway.
static void verifyMotorLedcTimers(const char *phase) {
  if (!motorPwmReady) return;

  const uint32_t leftHz = ledc_get_freq(
    (ledc_mode_t)LEDC_CHAN_GROUP(MOTOR_L_IN1_LEDC_CHANNEL),
    (ledc_timer_t)LEDC_CHAN_TIMER(MOTOR_L_IN1_LEDC_CHANNEL)
  );
  const uint32_t rightHz = ledc_get_freq(
    (ledc_mode_t)LEDC_CHAN_GROUP(MOTOR_R_IN1_LEDC_CHANNEL),
    (ledc_timer_t)LEDC_CHAN_TIMER(MOTOR_R_IN1_LEDC_CHANNEL)
  );

  char dbg[160];

  if (leftHz == MOTOR_LEDC_FREQ_HZ && rightHz == MOTOR_LEDC_FREQ_HZ) {
    snprintf(
      dbg,
      sizeof(dbg),
      "%s: Motor LEDC timers OK; left %u Hz, right %u Hz",
      phase,
      (unsigned int)leftHz,
      (unsigned int)rightHz
    );
    setDebugMessage(dbg);
    return;
  }
  ...
  snprintf(
    dbg,
    sizeof(dbg),
    "WARN: %s: motor LEDC timer clobbered; left %u Hz, right %u Hz, expected %u Hz",
    phase,
    (unsigned int)leftHz,
    (unsigned int)rightHz,
    (unsigned int)MOTOR_LEDC_FREQ_HZ
  );
  setDebugMessage(dbg);

  // Whatever took the timer, motors stuck at saturated duty is the worse
  // outcome. Stop first, reprogram, and leave the robot stopped: the operator
  // gets a still robot and a reason, instead of a wheel at full speed.
  lockMotors();
  stopMotorsUnlocked();
  motionState = MOTION_STOPPED;
  motorPwmReady = applyMotorLedcSetup();
  stopMotorsUnlocked();
  unlockMotors();

  setDebugMessage(
    motorPwmReady
      ? "WARN: motor LEDC timers reprogrammed; motors stopped"
      : "WARN: motor LEDC reprogramming failed; motor bridge left disabled"
  );
}

static void applyMotion() {
  // If the mutex could not be allocated at boot, fail safe: never energize
  // the bridge. stopMotorsUnlocked() is safe here because no motor-writing
  // path is allowed to proceed without the mutex.
  if (motorMutex == nullptr || !motorPwmReady) {
    stopMotorsUnlocked();
    return;
  }
  ...
  const int leftNow = leftMotorSpeed;
  const int rightNow = rightMotorSpeed;
  const MotionState stateNow = motionState;

  switch (stateNow) {
    case MOTION_FORWARD:
      writeMotorsUnlocked(leftNow, rightNow, false);
      break;

    case MOTION_BACKWARD:
      writeMotorsUnlocked(leftNow, rightNow, true);
      break;

    case MOTION_LEFT:
      writeMotorsUnlocked(0, rightNow, false);
      break;

    case MOTION_RIGHT:
      writeMotorsUnlocked(leftNow, 0, false);
      break;
...
static void serviceWifiApply() {
  ...
  // This is an explicit user-requested connection attempt. Temporarily add
  // station capability while keeping the fallback AP alive. Unlike the old
  // background retry loop, this happens only when the user presses Apply.
  if (!fallbackApActive) {
    if (!startFallbackAccessPoint()) return;
  }
  ...
  wifiApplyStartedMs = millis();
  wifiApplyInProgress = true;
  previousStaStatus = WL_DISCONNECTED;
}
...
// Acquire one frame without allowing a concurrent camera reinitialization.
// The stream server and control server are separate FreeRTOS tasks.
static camera_fb_t *cameraAcquireFrame() {
  if (!cameraReady || cameraRecovering) return nullptr;
  ...
  if (!fb) cameraCaptureFailureCount++;
  return fb;
}

// Hard-recover the OV2640 after repeated NULL frame returns. GPIO32 is the
// camera PWDN pin on the AI-Thinker ESP32-CAM; a short power-down clears sensor
// states that a simple browser reconnect cannot. The HTTP stream remains open
// while this runs, so a successful recovery does not create another client.
static bool recoverCameraInPlace() {
  if (cameraMutex == nullptr) return false;

  if (xSemaphoreTake(cameraMutex, pdMS_TO_TICKS(3000)) != pdTRUE) {
    setDebugMessage("CAMERA: Recovery deferred; camera lock busy");
    return false;
  }
  ...
  cameraReady = false;
  streamHeartbeatMs = millis();
  setDebugMessage("CAMERA: Capture stalled; starting sensor recovery");

  // A previous failed probe can leave partially allocated driver state. Begin
  // from a known state, then power-cycle GPIO32 before every probe attempt.
  esp_camera_deinit();

  esp_err_t err = ESP_FAIL;
  bool usedSafeClock = (cameraConfig.xclk_freq_hz <= 16000000);

  for (int attempt = 1; attempt <= CAMERA_RECOVERY_INIT_ATTEMPTS; attempt++) {
    esp_camera_deinit();

    pinMode(PWDN_GPIO_NUM, OUTPUT);
    digitalWrite(PWDN_GPIO_NUM, HIGH);
    delay(CAMERA_RECOVERY_POWERDOWN_MS);
    streamHeartbeatMs = millis();

    digitalWrite(PWDN_GPIO_NUM, LOW);
    delay(CAMERA_RECOVERY_BOOT_MS +
          (attempt - 1) * CAMERA_RECOVERY_ATTEMPT_BACKOFF_MS);
    streamHeartbeatMs = millis();

    // Some OV2640/ESP32-CAM combinations are more reliable at 16 MHz after a
    // sensor lockup. Keep normal 20 MHz for the first two attempts and only
    // fall back to 16 MHz on the final attempt.
    if (attempt == CAMERA_RECOVERY_INIT_ATTEMPTS) {
      cameraConfig.xclk_freq_hz = 16000000;
      usedSafeClock = true;
      setDebugMessage("CAMERA: Final recovery attempt uses safe 16 MHz XCLK");
    }
    ...
    snprintf(
      dbg,
      sizeof(dbg),
      "CAMERA: Recovery attempt %d/%d failed: 0x%x (%s)",
      attempt,
      CAMERA_RECOVERY_INIT_ATTEMPTS,
      (unsigned int)err,
      esp_err_to_name(err)
    );
    setDebugMessage(dbg);
  ...
  // Recovery re-runs esp_camera_init(), successfully or not, so the motor
  // timers are back in question exactly as they were at boot.
  verifyMotorLedcTimers("CAMERA");

  if (err == ESP_OK) {
    cameraReady = true;
    cameraRecoveryCount++;

    // Restore the user's saved sensor settings after the new driver instance
    // exists, then discard the first frames while exposure/clocking settles.
    loadCameraSettings();

    for (int i = 0; i < CAMERA_RECOVERY_WARMUP_FRAMES; i++) {
    ...
    snprintf(
      dbg,
      sizeof(dbg),
      "CAMERA: Recovery OK (%lu since boot)%s",
      (unsigned long)cameraRecoveryCount,
      usedSafeClock ? "; safe 16 MHz XCLK active" : ""
    );
    setDebugMessage(dbg);
  } else {
    // Leave the failed camera fully quiescent during the background retry
    // interval. The next attempt will wake it with a fresh PWDN cycle.
    esp_camera_deinit();
    pinMode(PWDN_GPIO_NUM, OUTPUT);
    digitalWrite(PWDN_GPIO_NUM, HIGH);
    ...
    snprintf(
      dbg,
      sizeof(dbg),
      "CAMERA: Recovery exhausted: 0x%x (%s); background retry scheduled",
      (unsigned int)err,
      esp_err_to_name(err)
    );
    setDebugMessage(dbg);
  ...
  lastCameraRecoveryAttemptMs = millis();
...
// If a hard recovery failed, do not require a browser reconnect to bring the
// sensor back. loop() retries at a quiet five-second cadence while the control
// UI and motor controls remain available.
static void serviceCameraRecovery() {
  if (otaInProgress || cameraReady || cameraRecovering) return;

  unsigned long now = millis();
  if (lastCameraRecoveryAttemptMs != 0 &&
      (uint32_t)(now - lastCameraRecoveryAttemptMs) <
        CAMERA_BACKGROUND_RECOVERY_INTERVAL_MS) {
    return;
  }

  recoverCameraInPlace();
}
...
static void printSerialDebugHistory() {
  ...
  Serial.printf("[LOG] firmware %s; %u retained event(s)\n",
                FIRMWARE_VERSION,
...
static void printSerialHelp() {
  Serial.println();
  Serial.printf("ESP32-CAM Robot %s UART0 Console\n", FIRMWARE_VERSION);
...
void setup() {
  // Leave the ESP32 brownout detector enabled. A supply dip should produce a
  // clean brownout reset, not undefined CPU execution and a corrupted double
  // exception backtrace.
  ...
  // Force all direction inputs low first, then keep the shared PWM enable HIGH
  // continuously. Motion is controlled only by the four LEDC direction duties.
  ...
  // Fixed LEDC allocation; the channel table near the top of this file has the
  // group/timer arithmetic and why it matters. No analogWrite() allocator is
  // used anywhere for motors. This runs before the camera, and the camera then
  // reprograms low-speed timer 0 -- which is why nothing here may use it, and
  // why verifyMotorLedcTimers() re-checks after esp_camera_init() returns.
  ...
  if (motorMutex == nullptr) {
    // There is no safe way to run concurrent HTTP/timeout motor writes without
    // this lock. Direction duties remain zero, so the bridge stays electrically
    // stopped even though the shared PWMA/PWMB line is held HIGH.
    ledcWrite(MOTOR_L_IN1_LEDC_CHANNEL, 0);
    ledcWrite(MOTOR_L_IN2_LEDC_CHANNEL, 0);
    ledcWrite(MOTOR_R_IN1_LEDC_CHANNEL, 0);
    ledcWrite(MOTOR_R_IN2_LEDC_CHANNEL, 0);
  ...
  {
    char buildDbg[80];
    snprintf(buildDbg, sizeof(buildDbg), "BOOT: Firmware version %s (%s %s)",
             FIRMWARE_VERSION, __DATE__, __TIME__);
    setDebugMessage(buildDbg);
  }
  setDebugMessage("BOOT: Brownout protection enabled");
  setDebugMessage("BOOT: UI features = motor sync, UART console, camera settings");
  if (!motorPwmReady) {
    setDebugMessage("ERROR: Motor LEDC setup failed; direction outputs held at zero");
  } else if (motorMutex == nullptr) {
    setDebugMessage("ERROR: Motor mutex allocation failed; direction outputs held at zero");
  } else {
    setDebugMessage("BOOT: Motor PWM LEDC 10-13 (timers 1,2; clear of camera XCLK timer 0); shared enable HIGH; 16-step soft-start");
  }
  ...
  if (cameraMutex == nullptr) {
    setDebugMessage("WARN: Camera mutex allocation failed; recovery disabled");
  }
  ...
  if (err != ESP_OK) {
    ...
    snprintf(
      dbg, sizeof(dbg),
      "WARN: Camera init failed: 0x%x (%s); UI will start and retry in background",
  ...
  // esp_camera_init() has run either way, and either way it may have touched
  // an LEDC timer. Say what the motor timers are before anything can drive.
  verifyMotorLedcTimers("BOOT");
...
<body>
  ...
  <aside id="debugSidebar" class="debug-sidebar" aria-label="Debug sidebar">
    ...
    <div class="debug-grid">
      <span class="label">Firmware</span><span id="dbgFirmware" class="value">--</span>
...
<svg class="brand-logo" aria-hidden="true" focusable="false"><use href="#wdiLogo"></use></svg>
  ...
  <script>
    ...
    const dbgFirmware = document.getElementById("dbgFirmware");
    ...
    const videoVersion = document.getElementById("videoVersion");
    ...
    const STREAM_RETRY_MIN_MS = 1500;
    ...
    // the stream; do not judge it before then. A healthy stream also has to go
    // genuinely quiet for a while, twice in succession, before we replace it.
    const STREAM_GRACE_MS = 6000;
    const STREAM_STALE_MS = 10000;
    const STREAM_BAD_CHECKS_BEFORE_RESTART = 2;

    // Switching briefly to another browser tab must not tear down the camera.
    // Apart from being visually annoying, closing/reopening an MJPEG socket is
    // one of the heaviest things the ESP32-CAM can be asked to do. Only release
    // the single stream slot only after the page has stayed hidden for 5 minutes.
    const STREAM_BACKGROUND_PAUSE_MS = 300000;
    ...
    let streamBackgroundPauseTimer = null;
    let streamBadHealthChecks = 0;
    ...
    function startStream() {
      ...
      // Do not create a new video socket for a page nobody can currently see.
      // visibilitychange will start it when the tab becomes visible again.
      if (document.hidden) {
        streamStopped = true;
        return;
      }

      // When firmware reports that the sensor itself is offline, opening port
      // 81 repeatedly only creates a reconnect storm. The control-server
      // health poll will call startStream() once background recovery succeeds.
      if (cameraAvailable === false) {
        streamStopped = true;
        setStreamStatus("Camera recovering on the robot...");
        return;
      }

      streamStopped = false;
      streamStartedAtMs = Date.now();
      streamBadHealthChecks = 0;
    ...
    async function checkStreamHealth() {
      ...
      const active = Boolean(data.streamActive);
      const heartbeatAgeMs = Number(data.streamHeartbeatAgeMs);
      const ready = data.cameraReady !== false;
      const recovering = Boolean(data.cameraRecovering);
      cameraAvailable = ready;

      if (!ready) {
        if (!streamStopped) stopStream();
        streamBadHealthChecks = 0;
        setVideoFps(null);
        setLinkState("novideo");
        setStreamStatus(
          recovering
            ? "Camera recovering on the robot..."
            : "Camera offline; firmware is retrying automatically..."
        );
        return;
      }

      // The sensor has come back in the background. Open exactly one stream
      // connection rather than waiting for an error/retry timer to hammer it.
      if (streamStopped && !document.hidden && streamRetryTimer === null) {
        startStream();
        return;
      }
      ...
      const fresh =
        ...
        (recovering || (
          ...
          heartbeatAgeMs >= 0 &&
          heartbeatAgeMs < STREAM_STALE_MS
        ));

      // FPS is telemetry, not a liveness test. A single delayed FPS sample used
      // to make the browser kill a perfectly good stream, creating the very
      // disconnect that the watchdog was supposed to recover from.
      applyTelemetry(data);
      setLinkState(fresh ? "live" : "novideo");

      if (streamStopped) return;

      if (fresh) {
        // Some browsers are inconsistent about firing <img> load events for a
        // never-ending multipart response. The robot heartbeat is definitive,
        // so clear a stale "Connecting..." message here as well.
        streamBadHealthChecks = 0;
        streamRetryDelayMs = STREAM_RETRY_MIN_MS;
        setStreamStatus("");
      }
      ...
      streamBadHealthChecks++;

      if (streamBadHealthChecks >= STREAM_BAD_CHECKS_BEFORE_RESTART) {
        streamBadHealthChecks = 0;
        scheduleStreamRestart(
          active
            ? "Camera stream stalled. Reconnecting..."
            : "Camera stream stopped. Reconnecting..."
        );
      }
    }
    ...
    document.addEventListener("visibilitychange", () => {
      clearTimeout(streamBackgroundPauseTimer);
      streamBackgroundPauseTimer = null;

      if (document.hidden) {
        // Short tab switches are common while driving/debugging. Keep the
        // existing socket alive for several minutes instead of disconnecting on every
        // Alt-Tab. A truly abandoned tab still releases the ESP32's single viewer.
        streamBackgroundPauseTimer = setTimeout(() => {
          streamBackgroundPauseTimer = null;

          if (document.hidden && !streamStopped) {
            stopStream();
            setStreamStatus("Stream paused while this tab is hidden.");
          }
        }, STREAM_BACKGROUND_PAUSE_MS);
        return;
      }

      // If the background timer never fired, the original stream is
      // still alive: keep it. Only open a new socket when we actually paused.
      if (!streamStopped) {
        setStreamStatus("");
        checkStreamHealth();
        return;
      ...
      // If we did release the old socket, give the ESP32 a short moment to
      // notice that close before taking the one stream slot again.
      streamRetryTimer = setTimeout(() => {
        streamRetryTimer = null;
        startStream();
      }, 750);
    ...
    exportDebugLog.addEventListener("click", () => {
      ...
      const header = [
        ...
        "",
        "STATUS",
        "Firmware: " + dbgFirmware.textContent,
      ...
      const firmwarePart = safeFilePart(dbgFirmware.textContent);

      link.href = url;
      link.download = robotName +
        (firmwarePart ? ("_" + firmwarePart) : "") +
        "_debug_" + stamp + ".txt";
    ...
    function applyFirmwareVersion(version) {
      const value = String(version || "--");
      heroBuild.textContent = value;
      videoVersion.textContent = value;
      dbgFirmware.textContent = value;
    }
    ...
    async function adoptRobotState(buildOnly) {
      ...
      if (data.build) {
        ...
        applyFirmwareVersion(data.firmwareVersion || data.build);
        buildFetched = true;
      }
...
static void checkIndexGzAsset() {
  // index_html_gz.h is generated as part of every numbered release. Do not
  // compare source-literal length at runtime: host/compiler newline handling
  // can make that check disagree on Windows even when the generated gzip is
  // correct. The release tool verifies the bytes before packaging.
  indexGzUsable = true;
  ...
  snprintf(
    dbg,
    sizeof(dbg),
    "BOOT: UI served gzipped; %u bytes (release asset verified)",
...
static esp_err_t wifi_handler(httpd_req_t *req) {
  ...
  if (httpd_query_key_value(query, "scan", value, sizeof(value)) == ESP_OK) {
    ...
    // A station interface is required for scanning, but merely enabling it
    // does not start an association. Keep the AP running throughout.
    WiFi.mode(WIFI_AP_STA);
    WiFi.setSleep(false);
    delay(120);
    ...
    // Never restart a failed configured network merely because the user
    // scanned. That old behavior reintroduced the same AP stalls we are
    // avoiding. If a station was genuinely connected before the scan, resume
    // it; otherwise leave STA idle until the user explicitly presses Apply.
    if (wasConnected) {
      WiFi.begin(ssid, password);
      previousStaStatus = WL_CONNECTED;
    }
...
static esp_err_t capture_handler(httpd_req_t *req) {
  // Use the same guarded acquisition path as the stream. On PSRAM boards the
  // second framebuffer allows a snapshot while MJPEG is live; on single-buffer
  // boards esp_camera_fb_get() simply waits for the stream to return its frame.
  camera_fb_t *fb = cameraAcquireFrame();

  if (!fb && !streamClientActive) {
    // Never deinitialize the camera while the MJPEG task may still own a
    // framebuffer. The stream performs its own safe in-place recovery.
    if (recoverCameraInPlace()) fb = cameraAcquireFrame();
  }
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  if (!cameraReady || cameraRecovering) {
    httpd_resp_set_status(req, "503 Service Unavailable");
    ...
    return httpd_resp_send(
      req,
      cameraRecovering
        ? "Camera is recovering; retry shortly."
        : "Camera is temporarily offline; firmware is retrying in the background.",
      HTTPD_RESP_USE_STRLEN
    );
  }
  ...
  while (res == ESP_OK) {
    fb = cameraAcquireFrame();

    if (!fb) {
      consecutiveCaptureFailures++;

      // A single NULL framebuffer is recoverable and should not destroy the
      // MJPEG socket. Give the DMA/sensor several frame periods to recover.
      if (consecutiveCaptureFailures <= CAMERA_CAPTURE_RETRIES_BEFORE_RECOVERY) {
        if (consecutiveCaptureFailures == 1) {
          Serial.println("CAMERA: Missed frame; waiting without reconnect");
        }
        delay(CAMERA_CAPTURE_RETRY_MS);
        continue;
      }

      // Repeated failures usually mean the sensor/DMA state was disturbed.
      // Reinitialize it while keeping this HTTP client attached.
      if (recoverCameraInPlace()) {
      ...
      Serial.println("CAMERA: Recovery unavailable; closing this stream while background recovery continues");
      res = ESP_FAIL;
    } else {
      consecutiveCaptureFailures = 0;
      if (fb->format == PIXFORMAT_JPEG) {
    ...
    // A completed JPEG made it through the HTTP stack. This is a much more
    // reliable liveness signal than waiting for the one-second FPS window.
    streamHeartbeatMs = millis();
    windowFrames++;
  ...
  snprintf(
    endDbg,
    sizeof(endDbg),
    "CAMERA: Stream ended after %lus (%lu since boot), err 0x%x (%s)",
...
static esp_err_t sendCameraStatus(httpd_req_t *req) {
  ScopedCameraLock lock(pdMS_TO_TICKS(1000));

  if (!lock.locked() || !cameraReady || cameraRecovering) {
    httpd_resp_set_status(req, "503 Service Unavailable");
    httpd_resp_set_type(req, "application/json");
    return httpd_resp_send(
      req,
      "{\"error\":\"Camera sensor temporarily unavailable\"}",
      HTTPD_RESP_USE_STRLEN
    );
  }
  ...
  // Snapshot while locked, then release before sending over Wi-Fi so a slow
  // client can never hold the camera mutex and starve the stream worker.
  framesize_t frameSize = sensor->status.framesize;
  unsigned int quality = (unsigned int)sensor->status.quality;
  int brightness = (int)sensor->status.brightness;
  int contrast = (int)sensor->status.contrast;
  int saturation = (int)sensor->status.saturation;
  unsigned int vflip = (unsigned int)sensor->status.vflip;
  unsigned int hmirror = (unsigned int)sensor->status.hmirror;
  lock.release();
...
static esp_err_t camera_handler(httpd_req_t *req) {
  ...
  if (httpd_query_key_value(
    ...
    char body[384];
    uint32_t heartbeatAgeMs = streamClientActive
      ? (uint32_t)(millis() - streamHeartbeatMs)
      : 0;
    ...
    snprintf(
      ...
      "\"streamHeartbeatAgeMs\":%lu,"
      "\"cameraReady\":%s,"
      "\"cameraRecovering\":%s,\"cameraRecoveries\":%lu,"
      "\"cameraCaptureFailures\":%lu,"
      "\"rssi\":%ld,\"rssiValid\":%s,"
  ...
  ScopedCameraLock cameraLock(pdMS_TO_TICKS(1000));
  if (!cameraLock.locked() || !cameraReady || cameraRecovering) {
    httpd_resp_set_status(req, "503 Service Unavailable");
    httpd_resp_set_type(req, "application/json");
    return httpd_resp_send(
      req,
...
#undef REGISTER_CONTROL_URI
  ...
  // A brief 2.4 GHz stall must not tear down a long-lived MJPEG connection.
  // The old five-second write timeout was unnecessarily aggressive on the
  // ESP32's fallback access point, especially while control traffic was busy.
